...
class FxsStickyLipDeformer_v2(omMPx.MPxDeformerNode):
    """Template deformer node."""
    # Replace this with a valid node id for use in production.
    type_id = om.MTypeId(0x00000007)  
    type_name = "FxsStickyLipDeformer_v2"
    ...
    def deform(
        ...
        upper_handle = data_block.inputValue(FxsStickyLipDeformer_v2.aUpperLipIndices)
        ...
        lower_handle = data_block.inputValue(FxsStickyLipDeformer_v2.aLowerLipIndices)
        ...
        while not mesh_vertex_iterator.isDone():
            ...
            if vertex_index in lower_indeces:
                ...
                upper_idx, upper_point = self.findClosestVertex(lower_point, upper_indices, mesh_fn)
    ...
    def findClosestVertex(self,lower_point, targetIndecies, mesh):
        """go find closest vertex to current Vertex"""

        minDist = float('inf')
        closestIndex = -1
        upper_point = om.MPoint()
    
        for i, targetIndex in enumerate(targetIndecies):
            mesh.getPoint(targetIndex, upper_point, om.MSpace.kWorld)
            
            distance_vector = lower_point - upper_point
        ...
        mesh.getPoint(targetIndecies[closestIndex], upper_point, om.MSpace.kWorld)
        return targetIndecies[closestIndex], upper_point
...
def initializePlugin(plugin):
    ...
    try:
        plugin_fn.registerNode(
            FxsStickyLipDeformer_v2.type_name,
            FxsStickyLipDeformer_v2.type_id,
            FxsStickyLipDeformer_v2.creator,
            FxsStickyLipDeformer_v2.initialize,
            omMPx.MPxNode.kDeformerNode
        )
    except:
        print("failed to register node {0}".format(FxsStickyLipDeformer_v2.type_name))
        raise
...
def uninitializePlugin(plugin):
    ...
    try:
        plugin_fn.deregisterNode(FxsStickyLipDeformer_v2.type_id)
    except:
...
gui_template = '''
    global proc AEFxsStickyLipDeformer_v2Template( string $nodeName )
    {
